...
//    }
    ...
    public mutating func remove(_ object: T) -> Index? {
        guard let index = contents[object] else {
            return nil
        }
        ...
        for (object, i) in contents where i >= index {
            contents[object] = i - 1
        }
    ...
    public mutating func swapObject(_ first: T, with second: T) {
        guard let firstPosition = contents[first], let secondPosition = contents[second] else {
            return
        }
